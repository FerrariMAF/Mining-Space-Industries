...
function msi_spawner_integration(scale)
return
  {
...
function msi_spawner_idle_animation(variation, tint,scale)
return
  {
    layers =
    {
      {
        ...
        shift = util.by_pixel(2*scale, -4*scale),
        y = variation * 180 * 2,
		scale=scale,
        hr_version =
        {
          ...
          direction_count = 1,
          run_mode = "forward-then-backward",
          shift = util.by_pixel(3*scale, -2*scale),
          y = variation * 354 * 2,
		  scale=scale,
          --scale = 0.5
        }
      },
      {
        ...
        animation_speed = 0.18,
        run_mode = "forward-then-backward",
        shift = util.by_pixel(-2*scale, -14*scale),
        line_length = 4,
        tint = tint,
        y = variation * 118 * 2,
		scale=scale,
        hr_version =
        {
          ...
          animation_speed = 0.18,
          run_mode = "forward-then-backward",
          shift = util.by_pixel(-1*scale, -14*scale),
          line_length = 4,
          tint = tint,
          y = variation * 234 * 2,
          --scale = 0.5
...
function msi_spawner_die_animation(variation, tint,scale)
return
  {
    layers =
    {
