...
function BuscaPosicaoAleatoria (px,py,TamanhoMin,TamanhoExtra,DistanciaCentro)

    local X=px - TamanhoMin - math.random(0,TamanhoExtra)
    local Y=py - TamanhoMin - math.random(0,TamanhoExtra)

    local XX=px + TamanhoMin + math.random(0,TamanhoExtra)
    local YY=py + TamanhoMin + math.random(0,TamanhoExtra)
	...
	for a=1,100 do
		...
		novoX=math.random(X,XX)
		novoY=math.random(Y,YY)

		
		--if novoX<PosCentral.x - TamanhoMin and novoX>PosCentral.x + TamanhoMin and novoY<PosCentral.y - TamanhoMin and novoY>PosCentral.y + TamanhoMin
		
		if distance_squared({novoX,novoY},{px,py})>DistanciaCentro then
			tipoterreno = global.surface.get_tile(novoX, novoY).name
